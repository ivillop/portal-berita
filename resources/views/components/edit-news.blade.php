...
<button data-modal-target="crud-modal-{{ $item->id }}" data-modal-toggle="crud-modal-{{ $item->id }}"
    class="bg-yellow-200 active:bg-yellow-300 p-1 rounded" type="button" href="/detail/{{ $item->id }}/edit">
...
<div id="crud-modal-{{ $item->id }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 p-4">
            ...
            <!-- Modal body -->
            <form action="/detail/{{ $item->id }}" method="POST" class="p-4 md:p-5">
                @csrf
                @method('PUT')
                ...
                <button type="submit"
                    class="text-gray-900 inline-flex items-center bg-yellow-200 active:bg-yellow-300 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
